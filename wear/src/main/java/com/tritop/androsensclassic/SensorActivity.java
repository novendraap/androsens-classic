...
public class SensorActivity extends Activity implements SensorEventListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.tritop.androsensclassic.R.layout.activity_sensor_wear);
        Intent intent = getIntent();
        if (intent != null) {
            index = intent.getIntExtra("SENSORINDEX", 0);
        }
        mSensor = ((AndrosensApp) this.getApplication()).getSensor(index);
        stub = (WatchViewStub) findViewById(com.tritop.androsensclassic.R.id.watch_view_stub);
